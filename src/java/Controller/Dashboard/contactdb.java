 ...
 * @author pc
 */
@WebServlet(name="contactdb", urlPatterns={"/contactdb"})
public class contactdb extends HttpServlet {
    ...
    List<Contact> list = countdao.pagingContact(index);
    ...
    throws ServletException, IOException {
      ...
      int count = dao.getTotalContactBySearch(NameSearch);
      int maxPage = (count / 5) + (count % 5 != 0 ? 1 : 0);
       List<Contact> lis= dao.getSearchContact(NameSearch,index);
         request.setAttribute("listCT", lis);
