...
<html lang="en">
    ...
    <head>
        ...
        <%
         List<Blog> eList = (List<Blog>)request.getAttribute("eList");
                ...
                <body>
                    <div class="container-fluid">
                        <div class="layout-specing">
                            <h2 style="text-align: center">Manager Blogs</h2>   
  ...
  if (eList == null) {} else {  
    for (int i = 0; i < eList.size(); i++) {
                                    %>  
                                    <tr onclick="window.location.href = 'manager_blog_details?blog_id=<%=eList.get(i).getId()%>'">
                                        <td style="text-align: left"> <%=eList.get(i).getName()%> </td>
                                        <td> <img src="<%=eList.get(i).getImage()%>" class="avatar avatar-md-sm rounded-circle shadow"> </td>
                                        <td style="text-align: center; font-size:14px "> <%=eList.get(i).getNameclub()%> </td>
                                        <td style="text-align: center; font-size:14px">
                                            <form action="Blog_delete" method="post" style="display: inline;">
                                                    <input type="hidden" name="id" value="<%=eList.get(i).getId()%>">
                                                <input type="hidden" name="status" value="<%=eList.get(i).getStatus() == 0 ? 1 : 0%>">
                                                <button type="submit" class="btn btn-icon btn-pills <%=eList.get(i).getStatus() == 0 ? "btn-soft-danger" : "btn-soft-success"%>"
                                                        style="color: <%=eList.get(i).getStatus() == 0 ? "red" : "green"%>;">
                                                    <i class="uil <%=eList.get(i).getStatus() == 0 ? "uil-multiply" : "uil-check-circle"%> text"></i>
                                                </button>
                                            </form>
                                        </td>
                                    </tr>
                            ...
                            <div style="text-align: center">
                                ...
                                <form style="display: inline-block;" action="manager_blogs" method="get">
